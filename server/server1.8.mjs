...
app.get('/get-rss', async (req, res) => {
  ...
  while (attempts < maxRetries && !success) {
    ...
    try {
      ...
      if (rssUrl) {
        ...
        // Click the "Save Feed" button after the RSS feed is generated
        await waitForTimeout(10000);  // Wait for a few seconds
        // Now click the "Save Feed" button
      await page.waitForSelector('button[ga="create-feed-save-and-redirect"]');
      await page.click('button[ga="create-feed-save-and-redirect"]');
      console.log('Feed saved successfully.');


        // Wait for the iFrame page to load and then click "Add to JavaScript"
        await waitForTimeout(10000);  // Wait for the save to complete
        await page.evaluate(() => {
          const iframeButtons = [...document.querySelectorAll('button')];
          const addToJsButton = iframeButtons.find(button => button.textContent.trim() === 'Add to JavaScript');
          if (addToJsButton) {
            addToJsButton.click();  // Click the "Add to JavaScript" button
          } else {
            console.log('Add to JavaScript button not found');
          }
        });

        // Extract the iFrame URL
        await waitForTimeout(3000);  // Wait for the iframe generation
        iframeUrl = await page.evaluate(() => {
          const iframeInput = document.querySelector('input[aria-invalid="false"][autocomplete="off"]');
          return iframeInput ? iframeInput.value : null;
        });

        console.log(`Iframe URL extracted: ${iframeUrl}`);
      } else {
  ...
  if (success && rssUrl && iframeUrl) {
    res.json({ rssUrl, iframeUrl });
  } else {
    res.status(500).json({ error: 'Failed to fetch the RSS feed or iframe after multiple attempts' });
  }
});
