...
app.get('/get-rss', async (req, res) => {
  ...
  while (attempts < maxRetries && !success) {
    ...
    try {
      ...
      await page.keyboard.type('div > div > div > div > div > div > div > main > div > div > div > div > div > div > div > div > div > div > div > div > div > div > a:nth-child(1)');  // Type the selector
      ...
      if (rssUrl) {
        ...
        throw new Error('RSS feed URL not found, retrying...');
      }
